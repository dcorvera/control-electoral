<script lang="ts">
  ...
  import {
    ...
    updateprovince,
    deleteprovince
  } from '$lib/services/provinceService';
  import { getCountry } from '$lib/services/countryService';
  import type { Province, Country, GeoPointData, PolygonData, Departament } from '$lib/types/types';
  ...
  let selected: Province | null = null;
  ...
  function validateForm() {
    errors = { code:'', name:'', countryId:'', departamentId:'', location:'', area:'' };
  ...
  onMount(async () => {
    if (browser) {
      ...
      allProvinces = await getprovinces();
  ...
  async function save() {
    ...
    try {
      ...
      if (modalType === 'add') {
        await createprovince(dataToSave);
      } else if (selected) {
        await updateprovince(selected.id || '', dataToSave);
      }
      await reload();
      close();
    } catch (error) {
  ...
  async function deleteItem() {
    ...
    await deleteprovince(selected.id || '');
    await reload();
    close();
  }
  ...
  $: filtered = allProvinces
    .filter(o => o.name.toLowerCase().includes(searchLower) || 
                 String(o.code).includes(search) || o?.departament?.name.toLowerCase().includes(searchLower) )
    .sort((a, b) => {
  ...
  function sort(key: keyof Province) {
    sortAsc = sortKey === key ? !sortAsc : true;
    sortKey = key;
  }

  function open(type: 'add' | 'edit' | 'delete', item?: Province) {
    modalType = type;
    if (type === 'add') {
      formData = { code:0, name:'', countryId:'', departamentId:'', location: undefined, area: undefined };
      selected = null;
    } else if (item) {
      // Al editar, cargar el país del departamento
      const dept = allDepartaments.find(d => d.id === item.departament?.id);
      formData = {
        code: item.code,
        name: item.name,
        countryId: dept?.country?.id || '',
        departamentId: item.departament?.id || '',
...
<section class="space-y-6 w-full max-w-full overflow-x-hidden">
  ...
  <div class="flex flex-col sm:flex-row gap-3 sm:gap-0 justify-between items-start sm:items-center">
    <h1 class="text-3xl font-bold dark:text-white">Provincias</h1>
    <button
      ...
      <table class="w-full table-auto min-w-[600px]">
        <thead class="bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900">
          <tr class="text-gray-700 dark:text-gray-300 uppercase text-xs">
            {#each [['id','ID'],['code','Código'],['name','Nombre'],['departament','Departamento']] as [key,label]}
              <th
                class="px-3 sm:px-6 py-2 sm:py-3 text-left cursor-pointer select-none"
                on:click={() => sort(key as keyof Province)}
              >
          ...
          {#each paginated as row, index (row.id)}
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition">
              ...
              <td class="px-3 sm:px-6 py-2 sm:py-4 dark:text-gray-200">{row.name}</td>
               <td class="px-3 sm:px-6 py-2 sm:py-4 dark:text-gray-200">{row.departament?.name}</td>
  ...
  <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white dark:bg-gray-900 border dark:border-gray-700 p-6 rounded-xl shadow-xl w-full max-w-sm sm:max-w-md max-h-[90vh] overflow-y-auto">
      {#if modalType === 'delete' && selected}
        <h2 class="text-xl font-bold mb-4 dark:text-white">Eliminar Provincia</h2>
      ...
      {:else}
        <h2 class="text-xl font-bold mb-4 dark:text-white">{modalType === 'add' ? 'Agregar' : 'Editar'} Provincia</h2>
