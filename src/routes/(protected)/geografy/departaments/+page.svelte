<script lang="ts">
  ...
  import {
    ...
    updateDepartament,
    deleteDepartament
  } from '$lib/services/departamentService';
  ...
  let loaded = false;
  let search = '';
  let sortKey: keyof Departament = 'id';
  let sortAsc = true;
  let currentPage = 1;
  const pageSize = 5;
  ...
  let selected: Departament | null = null;
  ...
  function validateForm() {
    errors = { code:'', name:'', countryId:'', location:'', area:'' };
    ...
    return !errors.code && !errors.name && !errors.countryId && !errors.location && !errors.area;
  }
  ...
  onMount(async () => {
    if (browser) {
      ...
      allDepartments = await getDepartaments();
  ...
  async function save() {
    ...
    try {
      if (modalType === 'add') {
        await createDepartament(formData);
      } else if (selected) {
        await updateDepartament(selected.id || '', formData);
      }
      await reload();
      close();
    } catch (error) {
  ...
  async function deleteItem() {
    ...
    await deleteDepartament(selected.id || '');
    await reload();
    close();
  }
  ...
  function sort(key: keyof Departament) {
    sortAsc = sortKey === key ? !sortAsc : true;
    sortKey = key;
  }

  function open(type: 'add' | 'edit' | 'delete', item?: Departament) {
    modalType = type;
    if (type === 'add') {
      formData = { code:0, name:'', countryId:'', location: undefined, area: undefined };
      selected = null;
    } else if (item) {
      formData = {
        code: item.code,
  ...
  function initPointMap() {
    ...
    // Si hay una ubicación previa, mostrar el marcador
  ...
  function initPolygonMap() {
    ...
    // Si hay un polígono previo, mostrarlo
  ...
  function initFullMap() {
    ...
    fullMap.on(L.Draw.Event.CREATED, (e: any) => {
      ...
      // Actualizar también el mapa pequeño
      if (drawnItems) {
        drawnItems.clearLayers();
...
<section class="space-y-6 w-full max-w-full overflow-x-hidden">
  ...
  <div class="flex flex-col sm:flex-row gap-3 sm:gap-0 justify-between items-start sm:items-center">
    <h1 class="text-3xl font-bold dark:text-white">Departamentos</h1>
    <button
      ...
      <table class="w-full table-auto min-w-[600px]">
        <thead class="bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900">
          <tr class="text-gray-700 dark:text-gray-300 uppercase text-xs">
            {#each [['id','ID'],['code','Código'],['name','Nombre']] as [key,label]}
              <th
                class="px-3 sm:px-6 py-2 sm:py-3 text-left cursor-pointer select-none"
                on:click={() => sort(key as keyof Departament)}
              >
  ...
  <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white dark:bg-gray-900 border dark:border-gray-700 p-6 rounded-xl shadow-xl w-full max-w-sm sm:max-w-md max-h-[90vh] overflow-y-auto">
      {#if modalType === 'delete' && selected}
        <h2 class="text-xl font-bold mb-4 dark:text-white">Eliminar Departamento</h2>
      ...
      {:else}
        <h2 class="text-xl font-bold mb-4 dark:text-white">{modalType === 'add' ? 'Agregar' : 'Editar'} Departamento</h2>
  ...
  <div class="fixed inset-0 bg-black/80 z-[60] flex items-center justify-center p-4">
    <div class="bg-white dark:bg-gray-900 rounded-lg p-4 w-full h-full max-w-6xl max-h-[90vh] relative">
      <div class="flex justify-between items-center mb-3">
        <h3 class="text-lg font-bold dark:text-white">Dibujar Área del Departamento</h3>
        <button
          type="button"
